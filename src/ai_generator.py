...
- 熱男スペシャル💎（絞り）は、必ず上で作成した「本線」の買い目の中から選んでください。本線に含まれない買い目を含めることは絶対禁止です。また、選ばれた買い目が重複しないように、必ず【ちょうど {bet_atsuo_max}点】を別々の買い目で出力してください。1点だけになったり、足りなかったりしないよう厳守してください。
- 抑えは {bet_osae_min}点 ～ {bet_osae_max}点 の範囲で出力。
- 【重要】「1=2-3」や「1-2=3」のような「=（折り返し）」での表記は一切禁止します。必ず「1-2-3」「2-1-3」のように、一つ一つの買い目を独立してハイフン区切りでのみ表記してください。
"""
        if fixed_first:
...
【出力要件・構成順序】
1. 締切時間とタイトル (例: {deadline}〆 ◯◯12R) ※「勝負」とは書かない
2. 各艇の注目ポイント（モーター・ST・当地実績など簡潔に）とレース展開予想。※句読点は絶対に使用しないこと！
